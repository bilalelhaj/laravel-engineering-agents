...
## Detected stack (before migration)
...
## Self-check
- [ ] Pre-migration suite was green (recorded above)
- [ ] Post-migration suite is green (or stopped on first regression with the failure surfaced)
- [ ] `composer.lock` matches the new `composer.json` versions
- [ ] No tests were softened to make them pass
- [ ] Asset rebuild ran if any asset config changed
- [ ] Every plugin's v-compat status is recorded in "Needs your decision"
- [ ] A single rollback commit-hash exists for the user
```
